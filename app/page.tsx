  ...
  return <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <main>
      ...
      <div className={`content ${tab === "builder" ? "builder-content" : ""}`}>
        {tab !== "builder" && <><div className="heading"><div><h1>Subscription</h1><small>Manage pricing plans, module access, and your public pricing page.</small></div><div className="heading-tools"><label className="search module-search"><Search /><input placeholder="Search customers, plans..." /><kbd>⌘ K</kbd></label><Button variant="outline" onClick={() => setTab("builder")}><Eye />Edit landing page</Button><Button onClick={() => setPlanDialog(true)}><Plus />New plan</Button></div></div><section className="stats"><article><i className="blue"><CreditCard /></i><span><b>{plans.length}</b><small>Total plans</small></span></article><article><i className="green"><Check /></i><span><b>{activePlans.length}</b><small>Published plans</small></span></article><article><i className="amber"><Boxes /></i><span><b>{modules.filter((m) => m.active).length}</b><small>Active modules</small></span></article><article><i className="violet"><Sparkles /></i><span><b>{plans.find((p) => p.recommended)?.name || "None"}</b><small>Recommended plan</small></span></article></section></>}
